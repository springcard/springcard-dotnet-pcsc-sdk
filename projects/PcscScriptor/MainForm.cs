...
namespace scscriptorxv
{
	...
	public partial class MainForm : Form
	{
		...
		void ReaderStatusChanged(uint ReaderState, CardBuffer CardAtr)
		{
			...
			if (InvokeRequired)
			{
				/* Avoid dirty exception when disconnecting savagely the device */
				try
				{
				...
				{
					Logger.Warning($"Ignore ThreadInterruptedException: {e.Message}");
				}
				return;
			}
			...
			else if ((ReaderState & SCARD.STATE_PRESENT) != 0)
			{
                ...
                if (Card == null)
				{
					...
					{
						Logger.Debug("Trying to connect to the card...");
						if (Card.Connect())
						{
						...
						{
							Logger.Debug("Card is already used");
							cardWasUsed = true;
							WriteLine();
							WriteLine("Waiting for card availibility", Color.Gray, false);
							Card = null;
						}
					}
				}
		...
		void SelectReader()
		{
			...
			if (rc)
			{
				...
				if (f.Mode == "ble")
				{
					...
					List<BluetoothUuid> deviceServicesUuid;

					f.GetBleParameters(out bleAdapter, out deviceAddress, out deviceServicesUuid);

					var listUuids = new List<string>();
					foreach(var uuid in deviceServicesUuid)
					{
						listUuids.Add(uuid.ToString());
					}
					
					SCardReaderList_CcidOverBle.BackgroundInstantiate(new SCardReaderList_CcidOverBle.BackgroundInstantiateCallback(OnBleReaderListInstantiated), bleAdapter, deviceAddress.ToString(), listUuids);
				}
			}
		}
		...
		void SaveToolStripMenuItemClick(object sender, EventArgs e)
		{
			...
			isSavingScript = true;
			saveFileDialog.FileName = ScriptFileName;
		...
		{
			File.WriteAllText(outputFileName, richTextBoxResult.Text);
